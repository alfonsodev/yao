...
func(q *Query) Get() ([]Modeler, error) {
	...
	var results []Modeler
	switch q.Table {
		case "users_orgs": results = Users_orgsGet(rows) 
		case "users": results = UsersGet(rows) 
		case "envs": results = EnvsGet(rows) 
		case "orgs": results = OrgsGet(rows) 
		case "teams": results = TeamsGet(rows) 
